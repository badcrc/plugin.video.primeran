...
OUTPUT_DIR = plugin.video.primeran-$(VERSION)
...
install:
	...
	-install -d $(OUTPUT_DIR)/resources/language/resource.language.eu_es/
	...
	install -m 644 resources/language/resource.language.eu_es/strings.po $(OUTPUT_DIR)/resources/language/resource.language.eu_es/strings.po


	zip -9 -r $(OUTPUT_FILE) $(OUTPUT_DIR)/
	- ln -s $(OUTPUT_DIR)/ plugin.video.primeran-latest
...
clean:
	...
	-rm plugin.video.primeran-latest
